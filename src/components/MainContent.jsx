...
function MainContent({
  ...
  onMenuClick,
  isLoading,
  isProjectSwitching, // Show loading state while switching projects
  onProjectReady, // Callback when project content is ready
  onInputFocusChange,
  ...
  // TaskMaster context
  const { tasks, currentProject, refreshTasks, setCurrentProject } =
    useTaskMaster();
  const { tasksEnabled, isTaskMasterInstalled, isTaskMasterReady } =
    useTasksSettings();
  ...
  // Handle project switching: signal ready after a brief delay to allow components to reset
  useEffect(() => {
    if (isProjectSwitching && selectedProject && onProjectReady) {
      // Give components time to reset/reload before showing content
      const timeoutId = setTimeout(() => {
        onProjectReady();
      }, 150);
      return () => clearTimeout(timeoutId);
    }
  }, [isProjectSwitching, selectedProject, onProjectReady]);

  // Show loading state during initial load or project switching
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center text-gray-500 dark:text-gray-400">
            ...
            <p>
              {isProjectSwitching
                ? `Loading ${selectedProject?.displayName || "project"}...`
          ...
          >
            <ErrorBoundary showDetails={true}>
              <ChatInterface
                ...
                onShowAllTasks={
                  tasksEnabled ? () => setActiveTab("tasks") : null
                }
              />
            </ErrorBoundary>
          </div>
